...
try {
    ...
    // 1. Kiểm tra xem hóa đơn mục tiêu có tồn tại, chưa bị xóa và đang ở trạng thái chờ duyệt hay không
    $queryCheck = "SELECT Id, TrangThaiThanhToan FROM HoaDon WHERE Id = :hd_id AND DeletedDate IS NULL";
    ...
    // Chỉ cho phép xóa khi hóa đơn ở trạng thái ChoDuyet (chờ duyệt)
    if ($invoice['TrangThaiThanhToan'] !== 'ChoDuyet') {
        http_response_code(400);
        echo json_encode(["status" => "error", "message" => "Chỉ được xóa hóa đơn ở trạng thái chờ duyệt (ChoDuyet). Hóa đơn hiện tại đang ở trạng thái: " . $invoice['TrangThaiThanhToan']], JSON_UNESCAPED_UNICODE);
        exit();
    }
