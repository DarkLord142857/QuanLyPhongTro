...
class LandlordInvoiceScreen extends StatefulWidget {
  ...
  final int? houseId;
  const LandlordInvoiceScreen({super.key, required this.landlordId, this.houseId});

  @override
  State<LandlordInvoiceScreen> createState() => _LandlordInvoiceScreenState();
}

class _LandlordInvoiceScreenState extends State<LandlordInvoiceScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchInvoices() async {
    ...
    try {
      ...
      final responses = await Future.wait([
        http.get(Uri.parse(invoiceUrl)).timeout(const Duration(seconds: 10)),
        http.get(Uri.parse(utilityUrl)).timeout(const Duration(seconds: 10)).catchError((_) => http.Response('{"status":"error","data":[]}', 200)),
      ]);
      ...
      _showSnackBar("Máy chủ phản hồi chậm!", Colors.orange);
    } catch (e) {
      debugPrint("Invoice Error: $e");
      _showSnackBar("Lỗi xử lý hóa đơn", Colors.red);
    } finally {
  ...
  Future<void> _updatePaymentStatus(int hoaDonId, String status) async {
    try {
      final response = await http.post(
        ...
        body: json.encode({"hoa_don_id": hoaDonId, "trang_thai": status, "ghi_chu": "Chủ trọ cập nhật trạng thái"}),
      );
      final data = json.decode(response.body);
      if (data['status'] == 'success') {
  ...
  void _showEditInvoiceDialog(dynamic item) async {
    ...
    try {
            ...
            builder: (ctx) => Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Sửa hóa đơn", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 15),
                    TextField(controller: roomPriceCtrl, decoration: const InputDecoration(labelText: "Tiền phòng", prefixIcon: Icon(Icons.home)), keyboardType: TextInputType.number),
                    const Divider(height: 30),
                    const Text("Chỉ số Điện", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.blue)),
                    ...
                    const SizedBox(height: 15),
                    TextField(controller: internetCtrl, decoration: const InputDecoration(labelText: "Phí dịch vụ khác", prefixIcon: Icon(Icons.wifi)), keyboardType: TextInputType.number),
                    const SizedBox(height: 30),
                    SizedBox(
                      width: double.infinity,
                      height: 50,
      ...
      if (mounted) Navigator.pop(context);
      _showSnackBar("Lỗi khi tải thông tin sửa", Colors.red);
    }
  }
  ...
  void _showStatusUpdateDialog(dynamic item) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text("Duyệt thanh toán", style: TextStyle(fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
  ...
  Widget _buildStatusUpdateOption(int id, String status, String label, Color color) {
    return ListTile(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Hóa Đơn Thu Tiền", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
      ...
      floatingActionButton: Column(
        ...
        children: [
          ...
          FloatingActionButton(
            heroTag: "refresh_invoices_landlord",
            ...
            children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text("Kỳ hóa đơn: ${item['Period']}", style: const TextStyle(color: Colors.grey, fontSize: 12)),
                            ],
                          ),
                        ),
                      ],
              ...
              Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    _buildTotalInfo("Tổng tiền", _formatCurrency(item['TotalPrice']), const Color(0xFF0F172A)),
                    _buildTotalInfo("Nợ cũ", _formatCurrency(item['Debt']), Colors.redAccent, crossAxisAlignment: CrossAxisAlignment.end),
                  ],
                ),
              ),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Expanded(child: Text("Khách: ${item['CustomerName'] ?? 'Ẩn danh'}", style: const TextStyle(fontSize: 13, color: Color(0xFF475569)), overflow: TextOverflow.ellipsis)),
                  Row(
                    children: [
                      if (status == 'choduyet') ...[
                        IconButton(
