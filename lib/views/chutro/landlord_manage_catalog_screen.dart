...
class LandlordManageCatalogScreen extends StatefulWidget {
  final int landlordId;
  const LandlordManageCatalogScreen({super.key, required this.landlordId});

  @override
  State<LandlordManageCatalogScreen> createState() => _LandlordManageCatalogScreenState();
}
...
class _LandlordManageCatalogScreenState extends State<LandlordManageCatalogScreen> {
  ...
  Future<void> _deleteService(int id) async {
    try {
      // Gửi Query Params theo đúng cấu trúc DELETE của file PHP của bạn
  ...
  void _showFormDialog(Map<String, dynamic>? item) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: Text(item == null ? "Thêm dịch vụ mới" : "Cập nhật dịch vụ", style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(controller: nameCtrl, decoration: const InputDecoration(labelText: "Tên dịch vụ *")),
              TextField(controller: costCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Đơn giá mẫu (VNĐ) *")),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Bảng giá dịch vụ mẫu"), backgroundColor: const Color(0xFF10B981), foregroundColor: Colors.white),
      body: _isLoading
              ...
              itemBuilder: (context, i) {
                ...
                return Card(
                  child: ListTile(
                    ...
                    subtitle: Text("${fmt.format(chiPhi)}\n${item['MoTa'] ?? ''}", style: const TextStyle(fontSize: 13)),
                    isThreeLine: true,
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        IconButton(icon: const Icon(Icons.edit, color: Colors.blue, size: 20), onPressed: () => _showFormDialog(item)),
                        IconButton(icon: const Icon(Icons.delete, color: Colors.red, size: 20), onPressed: () => _deleteService(int.parse(item['id'].toString()))),
                      ],
                    ),
                  ),
                );
