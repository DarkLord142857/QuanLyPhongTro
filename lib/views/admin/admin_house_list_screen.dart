...
class _AdminHouseListScreenState extends State<AdminHouseListScreen> {
  ...
  Future<void> _updateHouse(int id, String name, String address) async {
    try {
      ...
      final res = json.decode(response.body);
      if (res['status'] == 'success') {
        _showSnackBar("Cập nhật thông tin thành công!", Colors.blue);
        _refreshData();
      }
    } catch (e) {
  ...
  void _openHouseForm({HouseModel? house}) {
    ...
    final managerCtrl = TextEditingController(text: isEdit ? house.maQL.toString() : "2");
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            if(!isEdit) TextField(controller: managerCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "ID Chủ quản (MaQL)")),
          ],
        ),
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              if (nameCtrl.text.isEmpty || addrCtrl.text.isEmpty) return;
              Navigator.pop(ctx);
              if (isEdit) {
                _updateHouse(house.id, nameCtrl.text, addrCtrl.text);
              } else {
                _createHouse(nameCtrl.text, addrCtrl.text, int.tryParse(managerCtrl.text) ?? 2);
              }
            },
                ...
                child: Stack(
                  children: [
                    InkWell(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            const Icon(Icons.home_work_rounded, color: Colors.blue, size: 45),
                    ...
                    Positioned(
                      ...
                      child: Row(
                        children: [
                          IconButton(icon: const Icon(Icons.edit, size: 18, color: Colors.blue), onPressed: () => _openHouseForm(house: house)),
