...
class AdminServiceRequestScreen extends StatefulWidget {
  ...
  const AdminServiceRequestScreen({super.key, required this.landlordId});

  @override
  State<AdminServiceRequestScreen> createState() => _AdminServiceRequestScreenState();
}

class _AdminServiceRequestScreenState extends State<AdminServiceRequestScreen> {
  ...
  void _openAcceptDialog(int reqId) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        title: const Text("Admin Tiếp nhận"),
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => selectedId != null ? _updateStatus(reqId, selectedId!) : null, style: ElevatedButton.styleFrom(backgroundColor: Colors.blue), child: const Text("Bắt đầu xử lý", style: TextStyle(color: Colors.white)))
        ],
      ),
    );
  }
  ...
  void _openInvoiceDialog(int reqId, double defaultPrice) {
    ...
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text("Hoàn thành & Gửi hóa đơn (Admin)"),
        content: TextField(controller: ctrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: "Đơn giá thu (VNĐ)")),
        actions: [
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Yêu cầu dịch vụ (Admin)"), backgroundColor: Colors.blue, foregroundColor: Colors.white),
      body: _isLoading
                      ...
                      children: [
                        Row(
                          ...
                          children: [
                            ...
                            Text(status == 0 ? "Chờ xử lý" : (status == 1 ? "Đang sửa" : "Đã xong"), style: TextStyle(color: status == 0 ? Colors.red : (status == 1 ? Colors.orange : Colors.green), fontWeight: FontWeight.bold)),
                          ],
                        ),
                        const Divider(),
                        Text("Tiêu đề: ${item['TieuDe'] ?? ''}"),
                        Text("Mô tả: ${item['MoTa'] ?? ''}", style: const TextStyle(color: Colors.grey, fontSize: 13)),
                        const SizedBox(height: 12),
                        Row(
                          mainAxisAlignment: MainAxisAlignment.end,
                          children: [
                            if (status == 0) ElevatedButton(onPressed: () => _openAcceptDialog(int.parse(item['Id'].toString())), style: ElevatedButton.styleFrom(backgroundColor: Colors.blue), child: const Text("Tiếp nhận", style: TextStyle(color: Colors.white))),
                            if (status == 1) ElevatedButton(onPressed: () => _openInvoiceDialog(int.parse(item['Id'].toString()), cost), style: ElevatedButton.styleFrom(backgroundColor: Colors.orange), child: const Text("Bàn giao", style: TextStyle(color: Colors.white))),
                          ],
                        )
                      ],
                    ),
