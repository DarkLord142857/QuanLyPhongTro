...
class AdminRoomsScreen extends StatefulWidget {
  ...
  const AdminRoomsScreen({super.key, required this.adminId, this.houseId, this.onBackHome});
...
class _AdminRoomsScreenState extends State<AdminRoomsScreen> {
  ...
  // 🔥 ĐÃ ĐỒNG BỘ: Sử dụng đúng cấp thư mục của RoomDiscoveryController
  ...
  Future<void> _fetchHouses() async {
    try {
      // Sử dụng API chung GetHouses.php và gửi kèm adminId
      final response = await http.get(
        Uri.parse('$baseUrl/GetHouses.php?user_id=${widget.adminId}'),
  ...
  Future<List<RoomTenantModel>> _fetchRoomsWithTenants() async {
    final String url = '$baseUrl/GetRoom.php?house_id=${widget.houseId ?? ''}&user_id=${widget.adminId}';
    try {
      final response = await http.get(
        Uri.parse(url),
        headers: {"X-User-Id": widget.adminId.toString()},
      ...
      if (response.statusCode == 200) {
        ...
        return []; // Trả về danh sách trống nếu không có dữ liệu thay vì văng lỗi
      }
      throw Exception("Lỗi HTTP: ${response.statusCode}");
    } catch (e) {
      debugPrint("LỖI KẾT NỐI: $e");
      throw Exception("Lỗi kết nối: Không thể tải danh sách phòng.");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // Xóa AppBar ở đây vì AdminMainScreen đã cung cấp AppBar chung hoặc AdminHomeScreen đã có AppBar riêng
      floatingActionButton: FloatingActionButton(
        heroTag: "admin_add_room_fab",
                    ...
                    Wrap(
                      ...
                      children: room.danhSachThuocTinh.map((attr) {
                        return Container(
                          ...
                          child: Text("${attr.tenThuocTinh}: ${attr.giaTriThucTe}${attr.donVi ?? ''}", style: const TextStyle(fontSize: 11, color: Color(0xFF475569))),
                        );
                      }).toList(),
                    ),
  ...
  Widget _buildDefaultThumbnail() {
    return const Center(
      child: Column(
        ...
        children: [
          Icon(Icons.image_not_supported_rounded, size: 40, color: Color(0xFF94A3B8)),
          SizedBox(height: 8),
          Text("Chưa có hình ảnh", style: TextStyle(color: Color(0xFF94A3B8), fontSize: 12)),
        ],
      ),
    );
  }
  ...
  void _openRoomFormBottomSheet({RoomTenantModel? roomModel}) async {
    ...
    int? selectedHouseId = isEdit ? roomModel.nhaTroId : (widget.houseId ?? (_houses.isNotEmpty ? int.tryParse(_houses[0]['Id'].toString()) : null));
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(isEdit ? "Cập nhật phòng (Admin)" : "Thêm phòng mới (Admin)", 
                ...
                const Text("Nhà trọ", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 8),
                DropdownButtonFormField<int>(
                  value: selectedHouseId,
                  isExpanded: true, // Cho phép text dài không bị lỗi
                  ...
                  items: _houses.map((h) {
                    final int? id = int.tryParse(h['Id'].toString());
                    return DropdownMenuItem<int>(
                      value: id,
                      child: Text(h['TenNha'] ?? "Không tên", overflow: TextOverflow.ellipsis),
                    );
                  }).where((item) => item.value != null).toList(),
                  onChanged: isEdit ? null : (val) {
                    setModalState(() => selectedHouseId = val);
                  },
                  hint: const Text("Chọn nhà trọ"),
                  validator: (v) => v == null ? "Vui lòng chọn nhà trọ" : null,
                ),
                const SizedBox(height: 16),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text("Giá thuê", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                          const SizedBox(height: 8),
                          TextField(
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text("Người tối đa", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                          const SizedBox(height: 8),
                          TextField(
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text("Xe tối đa", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                          const SizedBox(height: 8),
                          TextField(
                ...
                const Text("Hình ảnh", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                const SizedBox(height: 8),
                SizedBox(
                  height: 100,
                  child: ListView(
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: isSubmitting 
                      ...
                      : Text(isEdit ? "CẬP NHẬT PHÒNG" : "TẠO PHÒNG TRỌ MỚI", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  ),
                ),
                const SizedBox(height: 20),
              ],
  ...
  void _showDeleteConfirmDialog(int id, String soPhong) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Bạn muốn xóa phòng $soPhong?"),
        actions: [
            ...
            onPressed: () async {
              ...
              try {
                final response = await http.post(
                  ...
                  body: json.encode({"Id": id, "NguoiXoaId": widget.adminId}) // Admin ID
                );
                final res = json.decode(response.body);
                if (res['status'] == 'success') {
                  _showSnackBar("Đã xóa thành công", primaryColor);
                  _refreshData();
                } else {
