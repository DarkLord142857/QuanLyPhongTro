...
try {
    ...
    $queryCheck = "SELECT TongTienHoaDon, CongNo, TrangThaiThanhToan FROM HoaDon WHERE Id = :hd_id AND DeletedDate IS NULL FOR UPDATE";
    ...
    // Chỉ cho phép sửa khi hóa đơn ở trạng thái ChoDuyet (chờ duyệt)
    if ($invoice['TrangThaiThanhToan'] !== 'ChoDuyet') {
        throw new Exception("Chỉ được sửa hóa đơn ở trạng thái chờ duyệt (ChoDuyet). Hóa đơn hiện tại đang ở trạng thái: " . $invoice['TrangThaiThanhToan']);
    }
